...
export const ListProjectGroups = () => {
  const projects = getProjects().filter(
    project => project.type === ProjectType.Curated
  ) as CuratedProject[]
  const orderedProjects = sortBy(x => x.projectGroup.id, projects)
  const projectGroups = groupWith(
    (projectA, projectB) =>
      projectA.projectGroup.id === projectB.projectGroup.id,
    orderedProjects
  )
  ...
  return sortBy(x => x[0].projectGroup.order, projectGroups).map(projects => {
    ...
    return (
      <div className="project-group" key={key}>
        <SubmenuItem url={urlFriendly(key)} id={urlFriendly(key)}>
          {key}
        </SubmenuItem>
        ...
        <Images
          images={sortBy(x => x.order, projects).map(createImagesFromProjects)}
        />
      </div>
    )
  })
