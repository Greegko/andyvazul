...
export const ListProjectsByGroup = () => {
  const projects = getProjects().filter(
    project => project.type === ProjectType.Artistic
  ) as ArtisticProject[]
  const groupedProjects: Record<string, ArtisticProject[]> = groupBy(
    x => x.group.toString(),
    projects
  )
  ...
  return (
    <div className="projects">
      {sortBy(getProjectMax, Object.entries(groupedProjects)).map(
        ([group, projects]) => (
          <div key={group}>
            <SubmenuItem url={urlFriendly(group)} id={urlFriendly(group)}>
              {group}
            </SubmenuItem>
            <div className="group-title">{group}</div>
            <Images
              images={sortBy(x => x.order, projects).map(
                createImagesFromProjects
              )}
            />
          </div>
        )
...
const getProjectMax = ([key, projects]: [string, Project[]]) =>
  Math.min(...projects.map(x => x.order))
