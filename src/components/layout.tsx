...
export const Layout = ({
  ...
  const [mainMenuPath, submenuPath, projectPath] = location.pathname
    .split("/")
    .splice(1)
  ...
  const [submenuItems, setSubmenuItems] = React.useState<MenuItem[]>(
    () => submenu || []
  )
  ...
  const addSubmenuItem = (item: MenuItem) =>
    setSubmenuItems(items => [
      ...items,
      { ...item, order: item.order ?? items.length },
    ])
  ...
  const displayCoreProperties: LayoutDisplayCore = {
    ...
    subsubmenuItems,
    mainMenuPath,
    contentPadding: projectPath
      ? subsubmenuMenuPos || activeMenuPos
      : activeMenuPos,
    activeSubmenu,
    activeSubSubmenu,
...
const getNextVisibleElement = (node: HTMLLinkElement): HTMLElement => {
  let next = node.nextElementSibling
    ...
    : (node.parentElement.nextElementSibling as HTMLElement)
...
const LayoutSubmenuScroll = (
  props: LayoutSubmenuScrollProperties & LayoutDisplayCore
) => {
  ...
  const submenuRef = React.useCallback(
    node => {
      if (!node) return
  ...
  const contentRef = React.useCallback(
    node => {
      ...
      const links = [
        ...node.querySelectorAll(".submenu-link"),
      ] as HTMLLinkElement[]
  ...
  React.useEffect(() => {
    ...
    const pos =
      ...
      getNextVisibleElement(lastLink).offsetTop -
      lastSubmenu.offsetTop -
      Padding
  ...
  React.useEffect(() => {
    ...
    const submenu = submenuLinks.find(
      x =>
        x.getAttribute("href").endsWith("/" + props.submenuPath) ||
        x.getAttribute("href").endsWith("/" + props.submenuPath + "/")
    )

    const menuPadding = submenu.offsetTop
      ? submenu.offsetTop
      : props.activeMenuPos
    const contentLink = contentLinks.find(
      x => x.id === "submenu-" + props.submenuPath
    )
    const pos = contentLink
      ? getNextVisibleElement(contentLink).offsetTop -
        (menuPadding - props.contentPadding)
      : 0
  ...
  React.useEffect(() => {
    ...
    const scrollLocationCheck = () => {
      const menuIndex = Math.max(
        0,
        contentLinks
          .map(getNextVisibleElement)
          .filter((x, index) => y(x) <= y(submenuLinks[index])).length - 1
      )
      const menuHref = submenuLinks[menuIndex].attributes.getNamedItem("href")
        .nodeValue
      const submenuPart = menuHref.split("/")[2]
...
const LayoutDisplay = (props: LayoutDisplayCore & LayoutDisplayScroll) => {
  ...
  const menuItemsWithTitle: MenuItem[] = [
    { path: "", title: "andyvazul", order: 0, style: { marginBottom: 30 } },
    ...menuItems,
  ]
  ...
  return (
    <div className={"layout" + (props.displayAsBlock ? " layout-block" : "")}>
      ...
      <div
        className={
          "sidebar" + (props.submenuItems.length > 0 ? " sidebar--submenu" : "")
        }
      >
        <div className="sidebar-content">
          <div className="menu">
            <Menu
              items={menuItemsWithTitle}
              activeItem={props.mainMenuPath}
              base=""
              setActiveElementPosition={props.setActiveMenuPos}
              padding={0}
            />
      ...
      <div
        ...
        className="content"
        style={{ paddingTop: props.contentPadding }}
      >
